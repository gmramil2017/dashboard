    ...
    {:card (pulse.test-util/checkins-query-card {:aggregation nil, :limit 9})
     ...
     (fn [_ thunk]
       (with-redefs [body/attached-results-text (pulse.test-util/wrap-function @#'body/attached-results-text)]
         (mt/with-temporary-setting-values [site-name "Dashboard Test"]
           (thunk))))
                     ...
                     {:blocks [{:type "divider"}
                               {:type "context"
                                :elements [{:type "mrkdwn"
                                            :text (str "<https://metabase.com/testmb/dashboard/"
                                                       dashboard-id
                                                       "?state=CA&state=NY&state=NJ&quarter_and_year=Q1-2021|*Sent from Dashboard Test*>")}]}]}]}
                   (pulse.test-util/thunk->boolean pulse-results)))))}})))
                 ...
                 {:blocks
                  [{:type "divider"}
                   {:type "context",
                    ...
                    [{:type "mrkdwn",
                      :text
                      #"<https://metabase\.com/testmb/dashboard/\d+\?state=CA&state=NY&state=NJ&quarter_and_year=Q1-2021\|\*Sent from Dashboard Test\*>"}]}]}]}
               (pulse.test-util/thunk->boolean pulse-results))))}}))
