            ...
            (with-redefs [request.u/geocode-ip-addresses (fn [ip-addresses]
              ...
              (mt/with-temp* [LoginHistory [_ {:user_id   user-id
                ...
                (is (schema= {(s/eq email)
                              [{:from    su/Email
                                :to      (s/eq [email])
                                :subject (s/eq (format "We've Noticed a New Dashboard Login, %s" first-name))
                ...
                (let [message (-> @mt/inbox (get email) first :body first :content)
                  ...
                  (testing (format "\nMessage = %s\nsite-url = %s" (pr-str message) (pr-str site-url))
                    (is (string? message))
                    (when (string? message)
                      (doseq [expected-str [(format "We've noticed a new login on your <a href=\"%s\">Dashboard</a> account."
                                                    (or site-url ""))
                                            (format "We noticed a login on your <a href=\"%s\">Dashboard</a> account from a new device."
                                                    (or site-url ""))
