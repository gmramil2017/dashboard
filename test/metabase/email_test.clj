  ...
  (tu/with-temporary-setting-values [email-from-address "lucky@metabase.com"
    ...
    (testing "basic sending"
      (is (=
           [{:from     (str (email/email-from-name) " <" (email/email-from-address) ">")
             :to       ["test@test.com"]
             :subject  "101 Reasons to use Dashboard"
             :reply-to (email/email-reply-to)
             :body     [{:type    "text/html; charset=utf-8"
                         :content "101. Dashboard will make you a better person"}]}]
           (with-fake-inbox
             (email/send-message!
              :subject      "101 Reasons to use Dashboard"
              :recipients   ["test@test.com"]
              :message-type :html
              :message      "101. Dashboard will make you a better person")
             (@inbox "test@test.com")))))
    ...
    (testing "with an attachment"
      (let [recipient    "csv_user@example.com"
            ...
            params       {:subject      "101 Reasons to use Dashboard"
                          :recipients   [recipient]
                          :message-type :attachments
                          :message      [{:type    "text/html; charset=utf-8"
                                          :content "100. Dashboard will hug you when you're sad"}
                                         {:type         :attachment
