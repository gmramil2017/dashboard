  ...
  (premium-features-test/with-premium-features #{}
    (ldap.test/with-ldap-server
      ...
      (try
        ...
        (testing "when givenName or sn attributes change in LDAP, they are updated in Dashboard on next login"
        ...
        (testing "if givenName or sn attributes are removed, values stored in Dashboard are updated to `nil` to respect the IdP response."
...
(deftest group-matching-test
  (testing "LDAP group matching should identify Dashboard groups using DN equality rules"
    (is (= #{1 2 3}
