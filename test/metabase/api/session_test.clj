...
(deftest properties-i18n-test
  (testing "GET /session/properties"
    (testing "Setting the X-Dashboard-Locale header should result give you properties in that locale"
      (mt/with-mock-i18n-bundles {"es" {:messages {"Connection String" "Cadena de conexión !"}}}
        (is (= "Cadena de conexión !"
               (-> (mt/client :get 200 "session/properties" {:request-options {:headers {"X-Dashboard-Locale" "es"}}})
