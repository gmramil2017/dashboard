...
(tx/defdataset-edn office-checkins
  "A small dataset that includes TIMESTAMP dates. People who stopped by the Dashboard office and the time they did so.")
...
(tx/defdataset-edn sample-dataset
  "The sample database that ships with Dashboard, but converted to an EDN dataset definition so it can be used in tests.
  This dataset is pretty large (over 20k rows) so it can take a long time to load -- keep that in mind. There is one
  difference from the H2 version that ships with Dashboard -- this version uses `:type/DateTimeWithTZ` `updated_at`
