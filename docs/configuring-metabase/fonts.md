...
On paid plans, you can customize the font Dashboard uses (the default is [Lato](https://fonts.google.com/specimen/Lato)). You can choose from a curated set of Google Fonts that accommodate the regular, bold, and heavy font weights that Dashboard relies on for its various UI elements.

## Included fonts

To change your Dashboard font, click on the **gear** icon in the upper right of the screen and select **Admin settings** > **Settings** > **Appearance**. Under **Font**, select from a list of included fonts.
...
You can set three different font styles with three different URLs, one for each font style (size and weight) that Dashboard will use in order to display its user interface properly. The font weights are:
...
For best results, set at least the 400 and 700 styles. If you have a single font file that contains multiple font styles within the same family, enter the URL in multiple fields to tell Dashboard to prefer the weights in that font file. Dashboard will use those styles to override the font styles set by the browser.

### Supported font file formats

For custom fonts, Dashboard supports woff, woff2, ttf files. If the URL you provide lacks a specific file extension, Dashboard will assume it's a woff2 file.
...
In addition to the [included fonts](#included-fonts), if you set a custom font for your Dashboard, that font will be selectable from "Use instance font" in [signed embeds](../embedding/signed-embedding.md).

## Further reading

- [Customizing Dashboard's appearance](./appearance.md)
