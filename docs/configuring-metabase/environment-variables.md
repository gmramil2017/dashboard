...
Many settings in Dashboard can be viewed and modified in the Admin Panel, or set via environment variables. The environment variables always take precedence. Note that the environment variables won't get written into the application database.

Setting environment variables can be done in various ways depending on how Dashboard is being run.
...
Enable the collection of anonymous usage data in order to help Dashboard improve.
...
Middleware that enforces validation of the client via the request header `X-Dashboard-Apikey`. If the header is available, then it’s validated against `MB_API_KEY`. When it matches, the request continues; otherwise it’s blocked with a 403 Forbidden response.

### `MB_APPLICATION_COLORS`

Only available on Dashboard [Pro](https://www.metabase.com/product/pro) and [Enterprise](https://www.metabase.com/product/enterprise) plans.<br>
Type: string<br>
Default: `"{}"`

JSON object of primary colors used in charts and throughout Dashboard. Examples:
...
Maximum number of connections to the Dashboard application database.

Change this to a higher value if you notice that regular usage consumes all or close to all connections. When all connections are in use, Dashboard might feel slow or unresponsive when clicking around the interface.

To see how many connections are being used, check the Dashboard logs and look for lines that contains the following: `… App DB connections: 12/15 …`. In this example, 12 out of 15 available connections are being used.

See [MB_JDBC_DATA_WAREHOUSE_MAX_CONNECTION_POOL_SIZE](#mb_jdbc_data_warehouse_max_connection_pool_size) for setting maximum connections to the databases connected to Dashboard.
...
Change the font used in Dashboard. See [fonts](../configuring-metabase/fonts.md).
...
Tell Dashboard which font files to use for different styles. Example value:
...
Replace the word `Dashboard` wherever it appears with the defined string.
...
Only available on Dashboard [Pro](https://www.metabase.com/product/pro) and [Enterprise](https://www.metabase.com/product/enterprise) plans.<br>
Type: integer<br>
Default: 0 (Dashboard keeps all rows)<br>

Sets the maximum number of days Dashboard preserves rows in the `query_execution` table in the application database. 

Twice a day, Dashboard will delete rows older than this threshold. 

The minimum value is `30` days (Dashboard will treat entered values of `1` to `29` the same as `30`). If set to `0`, Dashboard will keep all rows.
...
Identify when new versions of Dashboard are available.
...
When set to `false`, Dashboard will print migrations needed to be done in the application database and exit. Those migrations need to be applied manually. When `true`, Dashboard will automatically make changes to the application database. This is not related to migrating away from H2.
...
Timeout in milliseconds for connecting to databases, both Dashboard application database and data connections. In case you're connecting via an SSH tunnel and run into a timeout, you might consider increasing this value as the connections via tunnels have more overhead than connections without.
...
Address you want to use as the sender of emails generated by Dashboard, such as pulses or account invitations.
...
URL of origin allowed to embed the full Dashboard application.
...
When set, allows users to automatically create their Dashboard account by logging in if their Google account email address is from this domain.
...
Change this to a higher value if you notice that regular usage consumes all or close to all connections. When all connections are in use then Dashboard will be slower to return results for queries, since it would have to wait for an available connection before processing the next query in the queue.

See [MB_APPLICATION_DB_MAX_CONNECTION_POOL_SIZE](#mb_application_db_max_connection_pool_size) for setting maximum connections to the Dashboard application database.
...
Configure a host either as a host name or IP address to identify a specific network interface on which to listen. If set to `"0.0.0.0"`, Dashboard listens on all network interfaces. It will listen on the port specified in [MB_JETTY_PORT](#mb_jetty_port).
...
Change this to a higher value if you notice that regular usage consumes all or close to all threads. When all threads are in use Dashboard might feel slow or unresponsive when clicking around the interface.

To see how many threads are being used, check the Dashboard logs and look for lines that contain the following: `… Jetty threads: 45/50 …`, which in this case would indicate 45 out of 50 available threads are being used.
...
JSON object containing JWT to Dashboard group mappings. Should be in the form: `'{"groupName": [1, 2, 3]}'` where keys are JWT groups and values are lists of Dashboard groups IDs.
...
JSON object containing LDAP to Dashboard group mappings.
...
The base URL where dashboard notitification links will point to instead of the Dashboard base URL. Only applicable for users who utilize FullApp embedding and subscriptions.
...
Path of the "plugins" directory, which is used to store the Dashboard database drivers. The user who is running Dashboard should have permission to write to the directory. When running the JAR, the default directory is `plugins`, created in the same location as the JAR file. When running Docker, the default directory is `/plugins`.

The location is where custom third-party drivers should be added. Then Dashboard will load the driver on startup, which can be verified in the log.
...
Dashboard will cache all saved questions with an average query execution time longer than this many seconds.
...
JSON object containing SAML to Dashboard group mappings. Should be in the form: `'{"groupName": [1, 2, 3]}'` where keys are SAML groups and values are lists of Dashboard groups IDs.
...
Alias for the key that Dashboard should use for signing SAML requests.
...
Send email notification to user, when they login from a new device. Set to `false` to stop sending "We've noticed a new login on your Dashboard account" emails for all users.

Also, this variable controls the geocoding service that Dashboard uses to know the location from where your users logged in. Setting this variable to false also disables this reverse geocoding functionality.
...
Send email notifications to users in Admin group, when a new SSO users is created on Dashboard.
...
When using FullApp embedding, and the embedding website is hosted under a domain other than the one your Dashboard instance is hosted under, you most likely need to set it to `"none"`.
...
An UUID token used to signify that an instance has permissions to create the initial User. This is created upon the first launch of Dashboard, by the first instance; once used, it is cleared out, never to be used again.
...
The default language for this Dashboard instance. This setting applies to the Dashboard UI, system emails, [dashboard subscriptions](../dashboards/subscriptions.md), and [alerts](../questions/sharing/alerts.md). People can override the default language from their [account settings](../people-and-groups/account-settings.md).
...
The name used for this instance of Dashboard.
...
The base URL where users access Dashboard, e.g. `https://metabase.example.com` or `https://example.com/metabase`.

This URL is critical for things like SSO authentication, email links, embedding and more. Even difference with `http://` vs `https://` can cause problems. Make sure that the address defined is how Dashboard is being accessed.
...
Set the system files channel used by Dashboard to store images. This channel has to be public, and is not intended to be used by humans. The Slack App has to be invited into this channel.
