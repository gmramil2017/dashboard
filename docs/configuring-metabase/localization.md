...
The **Localization** settings allow you to set global defaults for your Dashboard instance. You can find **Localization** under **Admin settings** > **Settings**.

## Default language

Here you can set the default language (also called the "instance language") across your Dashboard UI, system [emails](./email.md), [dashboard subscriptions](../dashboards/subscriptions.md), and [alerts](../questions/sharing/alerts.md). People can pick a different language from their own [account settings](../people-and-groups/account-settings.md).

## Supported languages

Thanks to our amazing user community, Dashboard has been translated into many different languages. Due to [the way we collect translations](#translations), languages may be added or removed during major releases depending on translation coverage.
...
Our community contributes to Dashboard translations on our [POEditor project](https://poeditor.com/join/project/ynjQmwSsGh). If you'd like to help make Dashboard available in a language you're fluent in, we'd love your help!

For a new language to be added to Dashboard, it must reach 100%. Once it does, we add it in the next major or minor release of Dashboard. All _existing_ languages in Dashboard _must stay at 100%_ to continue being included in the next _major_ version of Dashboard. This rule ensures that no one encounters a confusing mishmash of English and another language when using Dashboard.
...
Use **report timezone** to set a default display time zone for dates and times in Dashboard. The report timezone setting is a display setting only, so changing the report timezone won't affect the time zone of any data in your database.
