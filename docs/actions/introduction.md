...
**Actions** are entities in Dashboard that let you build custom forms and business logic.
...
Just something to clarify here: actions, even though they are added to models, make their changes to the underlying table that a model queries. Which means that anyone who has access to the underlying table, or to questions or other models based on that table, will be able to see the effects of an action. Tools other than Dashboard that are connected to that database will also pick up these changes.
...
Before using actions in production, consider playing around with actions on some sample data (like the Sample Database included with Dashboard) to get a feel for how they work.

## Action gotchas

- If caching is enabled for the relevant table or model, you may not see the effects of an action in Dashboard until Dashboard refreshes the data (though you can always manually refresh the data).
