...
In general, embedding works by displaying a Dashboard URL inside an iframe on your website. A **signed embed** (or standalone embed) is an iframe that's secured with a signed JSON Web Token (JWT). The signed JWT will prevent website visitors from poking around in your Dashboard through the iframe.
...
If you want to set up interactive Dashboard filters in your iframe, your web server will need to make requests to Dashboard for updated data each time a website visitor updates the filter widget.

To ask for updated data from Dashboard, your web server will generate a new Dashboard [embedding URL](#adding-the-embedding-url-to-your-website). For example, if a website visitor enters the value "true" in an [embedded filter widget](./signed-embedding-parameters.md#adding-a-filter-widget-to-a-signed-embed), your web server will generate a new embedding URL with an extra parameter:

```
your_metabase_embedding_url?filter=true
```

To prevent people from editing the embedding URL to get access to other parts of your Dashboard (e.g., by changing the parameter to `filter=company_secrets`), your web server will add a signed JWT to the new embedding URL:

```
your_metabase_embedding_url/your_signed_jwt?filter=true
```

The signed JWT is generated using your [Dashboard secret key](#regenerating-the-secret-key). It tells Dashboard that the request for filtered data can be trusted, so it's safe to display the results at the new embedding URL.
...
## Enabling the embedding feature in Dashboard
...
The embedding URL for a question or dashboard is the Dashboard URL that'll be displayed in your website's iframe. It's generated by your web server using your [Dashboard site URL](../configuring-metabase/settings.md#site-url), [signed JWT](#how-signed-embedding-works), and [parameters](./signed-embedding-parameters.md):
...
2. [Update the code](#adding-the-embedding-url-to-your-website) on your server so that it matches the code generated by Dashboard (**Code** > top block).
...
You can change the way an embedded question or dashboard looks in an iframe (which won't change how it looks in your Dashboard instance). Settings include:
...
For global appearance settings, such as the colors and fonts used across your entire Dashboard instance, see [Customizing Dashboard's appearance](../configuring-metabase/appearance.md).

## Removing the "Powered by Dashboard" banner

![Powered by Dashboard](./images/powered-by-metabase.png)

The banner appears on signed embeds created with the Dashboard open source version. You'll need to upgrade to a [paid plan](https://www.metabase.com/pricing) to remove the banner.
...
Dashboards are a fixed aspect ratio, so if you'd like to ensure they're automatically sized vertically to fit their contents you can use the [iFrame Resizer](https://github.com/davidjbradshaw/iframe-resizer) script. Dashboard serves a copy for convenience:
