...
2. Organize people into Dashboard [groups](../people-and-groups/start.md).
...
## Enabling full-app embedding in Dashboard

1. Go to **Settings** > **Admin settings** > **Embedding**.
2. Click **Enable**.
3. Click **Full-app embedding**.
4. Under **Authorized origins**, add the URL of the website or web app where you want to embed Dashboard (such as `https://*.example.com`).

## Setting up embedding on your website

1. Create an iframe with a `src` attribute set to:
   - the [URL](#pointing-an-iframe-to-a-metabase-url) of the Dashboard page you want to embed, or
   - an [authentication endpoint](#pointing-an-iframe-to-an-authentication-endpoint) that redirects to your Dashboard URL.
2. Optional: Depending on the way your web app is set up, set [environment variables](../configuring-metabase/environment-variables.md) to:
   ...
   - [Embed Dashboard in a different domain](#embedding-metabase-in-a-different-domain).
   - [Secure your full-app embed](#securing-full-app-embeds).
3. Optional: Enable communication to and from the embedded Dashboard using supported [`postMessage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage) messages:
   - [From Dashboard](#supported-postmessage-messages-from-embedded-metabase)
   - [To Dashboard](#supported-postmessage-messages-to-embedded-metabase)
4. Optional: Set parameters to [show or hide Dashboard UI components](#showing-or-hiding-metabase-ui-components).

Once you're ready to roll out your full-app embed, make sure that people **allow** browser cookies from Dashboard, otherwise they won't be able to log in.

### Pointing an iframe to a Dashboard URL

Go to your Dashboard instance and find the page that you want to embed.

For example, to embed your Dashboard home page, set the `src` attribute to your [site URL](../configuring-metabase/settings.md#site-url), such as:

`http://metabase.yourcompany.com/`

To embed a specific Dashboard dashboard, use the dashboard's URL, such as:

`http://metabase.yourcompany.com/dashboard/1`

### Pointing an iframe to an authentication endpoint

Use this option if you want to send people directly to your SSO login screen (i.e., skip over the Dashboard login screen with an SSO button), and redirect to Dashboard automatically upon authentication.

You'll need to set the `src` attribute to your auth endpoint, with a `return_to` parameter pointing to the encoded Dashboard URL. For example, to send people to your SSO login page and automatically redirect them to `http://metabase.yourcompany.com/dashboard/1`:

```
https://metabase.example.com/auth/sso?return_to=http%3A%2F%2Fmetabase.yourcompany.com%2Fdashboard%2F1
```

If you're using [JWT](../people-and-groups/authenticating-with-jwt.md), you can use the relative path for the redirect (i.e., your Dashboard URL without the [site URL](../configuring-metabase/settings.md#site-url)). For example, to send people to a Dashboard page at `/dashboard/1`:
...
To make sure that your embedded Dashboard works in all browsers, put Dashboard and the embedding app in the same top-level domain (TLD). The TLD is indicated by the last part of a web address, like `.com` or `.org`.

Note that your full-app embed must be compatible with Safari to run on _any_ browser in iOS (such as Chrome on iOS).

## Embedding Dashboard in a different domain

> Skip this section if your Dashboard and embedding app are already in the same top-level domain (TLD).

If you want to embed Dashboard in another domain (say, if Dashboard is hosted at `metabase.yourcompany.com`, but you want to embed Dashboard at `yourcompany.github.io`), you can set the following [environment variable](../configuring-metabase/environment-variables.md):

`MB_SESSION_COOKIE_SAMESITE=None`

If you set this environment variable to "None", you must use HTTPS in Dashboard to prevent browsers from rejecting the request. For more information, see MDN's documentation on [SameSite cookies](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie/SameSite). 
...
Dashboard uses HTTP cookies to authenticate people and keep them signed into your embedded Dashboard, even when someone closes their browser session. If you enjoy diagrammed auth flows, check out [Full-app embedding with SSO](https://www.metabase.com/learn/customer-facing-analytics/securing-embeds#full-app-embedding-with-sso).
...
To manually log someone out of Dashboard, load the following URL (for example, in a hidden iframe on the logout page of your application):
...
## Supported postMessage messages _from_ embedded Dashboard

To keep up with changes to an embedded Dashboard URL (for example, when a filter is applied), set up your app to listen for "location" messages from the embedded Dashboard. If you want to use this message for deep-linking, note that "location" mirrors "window.location".

```
{ "metabase": { "type": "location", "location": LOCATION_OBJECT_OR_URL }}
```

To make an embedded Dashboard page (like a question) fill up the entire iframe in your app, set up your app to listen for a "frame" message with "normal" mode from Dashboard:

```
{ "metabase": { "type": "frame", "frame": { "mode": "normal" }}}
```

To specify the size of an iframe in your app so that it matches an embedded Dashboard page (such as a dashboard), set up your app to listen for a "frame" message with "fit" mode from Dashboard:

```
{ "metabase": { "type": "frame", "frame": { "mode": "fit", height: HEIGHT_IN_PIXELS }}}
```

## Supported postMessage messages _to_ embedded Dashboard

To change an embedding URL, send a "location" message from your app to Dashboard:

```
{ "metabase": { "type": "location", "location": LOCATION_OBJECT_OR_URL }}
```

## Showing or hiding Dashboard UI components
...
For example, you can disable Dashboard's [top nav bar](#top_nav) and [side nav menu](#side_nav) like this:
...
Whether to show the logo that opens and closes the sidebar nav. Default is true. How Dashboard displays the logo depends on the `side_nav` setting. Here's a rough breakdown of how these two parameters interact:

If `logo=true` and:

- `side_nav=true`: Looks like regular Dashboard (with whatever logo you have set).
- `side_nav=false`: There is no sidebar, so nothing happens when you hover over the logo.

If `logo=false` and:

- `side_nav=true`: Dashboard shows the generic sidebar icon, with a gray color in normal state, and a brand color on hover.
...
`search`, `new_button`, and `breadcrumbs` all depend on `top_nav` being set to `true`. If these three children (`search`, `new_button`, and `breadcrumbs`) are all false, Dashboard will hide the top nav bar.
