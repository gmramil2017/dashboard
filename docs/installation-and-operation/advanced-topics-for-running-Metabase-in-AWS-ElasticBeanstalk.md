---
title: Advanced topics for running Dashboard on Elastic Beanstalk
redirect_from:
  - /docs/latest/operations-guide/advanced-topics-for-running-Dashboard-in-AWS-ElasticBeanstalk
---

# Advanced topics for running Dashboard on Elastic Beanstalk

## Logging

### Network Access log

If you need a log of all the IP addresses and URLs that were accessed during a specific period, you can configure the Load Balancer to send those logs to S3. This is useful for analyzing the traffic to your Dashboard instance.
...
If you want to retain the Dashboard application logs, you can publish them to an S3 bucket:

- On your Dashboard Elastic Beanstalk environment, click on the `Configuration` link in the navigation bar on the left side. On the configuration page, click on the `Edit` button next to `Software`.
...
## Running Dashboard over HTTPS

There is no requirement to run Dashboard over HTTPS, but we are sticklers for security and believe you should always be careful with your data. Here's how to set up HTTPS on AWS.
...
- Click on **Hosted Zones**, then click on the domain name you want to use for Dashboard.
- Click on the blue button **Create Record** (a new panel will open up).
  - Enter in a **Record name**: for your application. This record name should be the exact URL you plan to access Dashboard with (e.g. `metabase.mycompany.com`).
...
### Modify Dashboard to enforce HTTPS
...
- Click on **SSL certificate ID** and choose the name of the certificate that you uploaded.
  - The certificate MUST match the domain you plan to use for your Dashboard install.
...
Once this change is made you will no longer be able to access your Dashboard instance at the \*.elasticbeanstalk.com URL provided by Amazon because it will result in a certificate mismatch. To continue accessing your secure Dashboard instance you must [Set up a DNS CNAME](#setup-dns-cname-using-aws).
...
- Then, a little bit lower on the dropdown for `SSL certificate ID`, choose the name of the certificate that you uploaded to your account.
  - _NOTE: The certificate MUST match the domain you plan to use for your Dashboard install._
- In SSL Policy select `ELBSecurityPolicy-TLS-1-2-2017-01`
- Scroll to the bottom of the page and click `Save` in the lower right.
  ...
  - _IMPORTANT: Once this change is made you will no longer be able to access your Dashboard instance at the `*.elasticbeanstalk.com` URL provided by Amazon because it will result in a certificate mismatch. To continue accessing your secure Dashboard instance you must [Setup a DNS CNAME](#setup-dns-cname-using-aws)._
...
Dashboard installs the CloudWatch agent into the Elastic Beanstalk deployment, which sends data about your deployment to CloudWatch, allowing you to track your Dashboard's RAM usage and other metrics.
