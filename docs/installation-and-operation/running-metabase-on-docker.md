...
Dashboard provides an official Docker image via Dockerhub that can be used for deployments on any system that is running Docker.

If you're trying to upgrade your Dashboard version on Docker, check out these [upgrading instructions](upgrading-metabase.md).

## Open Source quick start

Use this quick start to run the Open Source version of Dashboard locally. See below for instructions on [running Dashboard in production](#production-installation).
...
Use this quick start if you have a [license token](../paid-features/activating-the-enterprise-edition.md) for a [paid version](https://www.metabase.com/pricing) of Dashboard, and you want to run Dashboard locally. See below for instructions on [running Dashboard in production](#production-installation).
...
Here's an example Docker command that tells Dashboard to use that database:
...
Keep in mind that Dashboard will be connecting from _within_ your Docker container, so make sure that either: a) you're using a fully qualified hostname, or b) that you've set a proper entry in your container's `/etc/hosts file`.
...
You can use any of the custom settings from [Customizing the Dashboard Jetty Webserver](../configuring-metabase/customizing-jetty-webserver.md) by setting environment variables in your Docker run command.
...
When you launch your container, Dashboard will use the database file (`MB_DB_FILE`) at `~/metabase-data/metabase.db` instead of its default location. and we are mounting that folder from our local filesystem into the container.

### Getting your config back if you stopped your container

If you've previously run and configured your Dashboard using the local Database and then stopped the container, your data will still be there unless you deleted the container with the `docker rm` command. To recover your previous configuration:
...
You should have your previously configured Dashboard Installation back. If it's not the one you expected, try a different stopped container and repeat these steps.

### Adding external dependencies or plugins

To add external dependency JAR files, such as the Oracle or Vertica JDBC drivers or 3rd-party Dashboard drivers), you'll need to:

- create a `plugins` directory in your host system, and
- bind that directory so it's available to Dashboard as the path `/plugins` (using either `--mount` or `-v`/`--volume`).

For example, if you have a directory named `/path/to/plugins` on your host system, you can make its contents available to Dashboard using the `--mount` option as follows:
...
Note that Dashboard will use this directory to extract plugins bundled with the default Dashboard distribution (such as drivers for various databases such as SQLite), thus it must be readable and writable by Docker.
...
Here is an example `docker-compose.yml` file to start a Dashboard Docker container with secrets to connect to a PostgreSQL database.
...
In order for the Dashboard container to read the files and use the contents as a secret, the environment variable name needs to be appended with a "\_FILE" as explained above.
