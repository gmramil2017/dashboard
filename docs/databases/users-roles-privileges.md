...
- Optional [`metabase_actions` for write access](#privileges-to-enable-actions) to tables used for Dashboard actions.
- Optional [`metabase_model_caching` for write access](#privileges-to-enable-model-caching) to the schema used for Dashboard model caching.
...
In order to view and query your tables in Dashboard, you'll have to give Dashboard's database user:

- `CONNECT` to your database.
- `SELECT` privileges to any schemas or tables that you want to use in Dashboard.
...
Depending on how you use Dashboard, you can also additonally grant:
...
[Actions](../actions/introduction.md) let Dashboard write back to specific tables in your database.
...
- Give the role `INSERT`, `UPDATE`, and `DELETE` privileges to any tables used with Dashboard actions.
- Give the `metabase_actions` role to the `metabase` user.

```sql
-- Create a role to bundle database privileges for Dashboard actions.
CREATE ROLE metabase_actions WITH LOGIN;

-- Grant write privileges to the TABLE used with Dashboard actions.
...
[Model caching](../data-modeling/models.md#model-caching) lets Dashboard save query results to a specific schema in your database. Dashboard's database user will need the `CREATE` privilege to set up the dedicated schema for model caching, as well as write access (`INSERT`, `UPDATE`, `DELETE`) to that schema.
...
```sql
-- Create a role to bundle database privileges for Dashboard model caching.
...
-- Create a role to bundle privileges to Dashboard actions for customer Lemon.
...
- If customer Lemon decides that they don't want to use Dashboard actions anymore (but they still want to ask questions), you can simply revoke or drop the `lemon_actions` role.
