...
H2 is a lightweight, in-memory database; it’s perfect for getting spun up quickly, but not so good for long-term usage. By default, Dashboard uses H2 to store its internal data. The Sample Database included with Dashboard is also an H2 database.

## Avoid using H2 as your application database in production

If you want to run Dashboard in production, we recommend that you [migrate off H2](../../installation-and-operation/migrating-from-h2.md).
...
The local path relative to where your Dashboard is running from. Your string should exclude the `.mv.db` extension.
