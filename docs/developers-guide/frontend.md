...
- `import`s should be ordered by type, typically:
  ...
  2. Dashboard's top-level React components and containers (`metabase/components/*`, `metabase/containers/*`, etc)
  3. Dashboard's React components and containers specific to this part of the application (`metabase/*/components/*` etc)
  4. Dashboard's `lib`s, `entities`, `services`, Redux files, etc
...
In Dashboard core, they are visually responsive: they appear above or below the element that triggers their appearance. Their height is automatically calculated to make them fit on the screen.
