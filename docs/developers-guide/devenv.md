...
The Dashboard application has two basic components:
...
Both components are built and assembled together into a single JAR file. In the directory where you run the JAR, you can create a JAR file (if Dashboard hasn't already created it) and add drivers in there (the drivers are also JARs).
...
Dashboard depends on third-party libraries to run, so you'll need to keep those up to date. The Clojure CLI will automatically fetch the dependencies when needed. With JavaScript dependencies, however, you'll need to kick off the installation process manually.
...
Most of the drivers Dashboard uses to connect to external data warehouse databases are separate projects under the
`modules/` subdirectory. When running Dashboard via `clojure`, you'll need to build these drivers in order to have access
...
drivers' dependencies and source paths into the Dashboard project:
