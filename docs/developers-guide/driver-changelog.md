...
- `metabase.driver.sql-jdbc.execute/connection-with-timezone` has been marked deprecated and is scheduled for removal
  in Dashboard 0.50.0. The new method `metabase.driver.sql-jdbc.execute/do-with-connection-with-options` replaces it.
...
- The multimethods `set-role!`, `set-role-statement`, and `default-database-role` have been added. These methods are
  ...
  be restricted at the database level instead of (or in conjunction with) Dashboard's built-in permissions system.

## Dashboard 0.46.0

- The process for building a driver has changed slightly in Dashboard 0.46.0. Your build command should now look
  something like this:

  ```sh
  # Example for building the driver with bash or similar

  # switch to the local checkout of the Dashboard repo
...
- The multimethod `metabase.driver/table-rows-sample` has been added. This method is used in situations where Dashboard
...
Prior to Dashboard 0.46.0, SQL drivers used Honey SQL 1 as an intermediate target when compiling queries. In 0.46.0 we
have began the process of migrating to Honey SQL 2 as our new intermediate target.

We plan to continue to support use of Honey SQL 1 until Dashboard 0.49.0. Please be sure to migrate your drivers before
then.

In Dashboard 0.46.x, 0.47.x, and 0.48.x, you can specify which version of Honey SQL you driver should use by
...
In Dashboard 0.47.x or 0.48.x we will likely change the default Honey SQL version to `2` to ensure everyone is aware of
...
4. In Honey SQL 1 you were able to register functions to a more limited extent by implementing the multimethod
   `honeysql.format/fn-handler`. Dashboard registered the functions `:extract`, `:distinct-count`, and
...
- `metabase.driver.sql-jdbc.connection/details->connection-spec-for-testing-connection` has been removed in Dashboard
...
- The `:expressions` map in an MBQL query now uses strings as keys rather than keywords (see
  ...
  available since at least Dashboard 0.35.0 and handles both types of keys. It is highly recommended that you use this
...
Changes in Dashboard 0.42.0 affect drivers that derive from `:sql` (including `:sql-jdbc`).
...
The following methods and vars are slated for removal in Dashboard 0.45.0 unless otherwise noted.
...
- `metabase.driver.sql.query-processor/field->alias`, which was deprecated in 0.41.0, is now unused in 0.42.0.
  ...
  for one reason or another. This method is still slated for removal in Dashboard 0.44.0.
...
| Version | Wiki page |
|---------|-----------|
| 0.41.0 | [changes](https://github.com/metabase/metabase/wiki/What's-new-in-0.41.0-for-Dashboard-driver-authors) |
| 0.40.0 | No changes. |
| 0.39.0 | No changes. |
| 0.38.0 | [changes](https://github.com/metabase/metabase/wiki/What's-new-in-0.38.0-for-Dashboard-driver-authors) |
| 0.37.0 | [changes](https://github.com/metabase/metabase/wiki/What's-new-in-0.37.0-for-Dashboard-driver-authors) |
| 0.36.0 | [changes](https://github.com/metabase/metabase/wiki/What's-new-in-0.36.0-for-Dashboard-driver-authors) |
| 0.35.0 | [changes](https://github.com/metabase/metabase/wiki/What's-new-in-0.35.0-for-Dashboard-driver-authors) |
