...
We are an application with lots of users all over the world. To help them use Dashboard in their own language, we mark all of our strings as i18n.
...
Dashboard allows for translations into many languages. An authoritative list can be found in `resources/locales.clj`.

## The Dashboard Side

Dashboard is concerned about localization into two distinct locales: translating into the server's locale and translating into the user's locale. The distinction is largely: will this be logged on the server or sent over the wire back to the user.
