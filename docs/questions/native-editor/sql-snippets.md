...
Here's a simple query with a join using the **Sample Database** included with Dashboard.
...
When writing in the SQL editor, you can now start typing `{% raw %}{{snippet:}}{% endraw %}` and Dashboard will present autocomplete options for available snippets.
...
Click on the snippet icon on the right side of the SQL editor, below the Data Reference book icon and the Variables χ icon. Dashboard will slide out a sidebar menu that lists available snippets.
