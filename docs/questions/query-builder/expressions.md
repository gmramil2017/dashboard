...
- **Filters**. The expression `= contains([comment], "Dashboard")` would filter for rows where the `comment` field contained the word "Dashboard".
...
This page covers the basics of expressions. You can check out a [full list of expressions][expression-list] in Dashboard, or walk through a tutorial that shows you how you can use [custom expressions in the notebook editor][custom-expressions].
...
- Filter expressions are different in that they must return a Boolean value (something that's either true or false). For example, you could write `[Subtotal] + [Tax] < 100`. Dashboard would look at each row, add its subtotal and tax, the check if that sum is greater than 100. If it is, the statement evaluates as true, and Dashboard will include the row in the result. If instead you were to (incorrectly) write `[Subtotal] + [Tax]`, Dashboard wouldn't know what to do, as that expression doesn't evaluate to true or false.
