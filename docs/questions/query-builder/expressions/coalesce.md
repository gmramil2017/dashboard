...
Calculations in Dashboard will return `null` if any of the input columns are `null`. This is because `null` values in your data represent "missing" or "unknown" information, which isn't necessarily the same as an amount of "0". That is, adding 1 + "unknown" = "unknown".
...
This section covers functions and formulas that can be used interchangeably with the Dashboard `coalesce` expression, with notes on how to choose the best option for your use case.
...
The [Dashboard `case` expression](./case.md)
...
is equivalent to the Dashboard `coalesce` expression:
...
are equivalent to the Dashboard `coalesce` expression:
