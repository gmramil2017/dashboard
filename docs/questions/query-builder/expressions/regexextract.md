...
`regexextract` is not supported on H2 (including the Dashboard Sample Database), SQL Server, and SQLite.

## Related functions

This section covers functions and formulas that work the same way as the Dashboard `regexextract` expression, with notes on how to choose the best option for your use case.
...
is equivalent to the Dashboard `regexextract` expression:
...
uses pretty much the same syntax as the Dashboard expression:
...
does the same thing as the Dashboard `regexextract` expression:
