...
is equivalent to the Dashboard expression:
...
The `SELECT` part of the SQl query matches the Dashboard expression:

```
CountIf([Active Subscription] = false)
```

The `GROUP BY` part of the SQL query matches a Dashboard [**Group by**](../../query-builder/introduction.md#summarizing-and-grouping-by) set to the "Plan" column.
...
produces the same result as the Dashboard expression:
...
uses the same logic as the Dashboard expression:
...
The Python code above will produce the same result as the Dashboard `CountIf` expression (with the [**Group by**](../../query-builder/introduction.md#summarizing-and-grouping-by) column set to "Plan").
