...
To start using alerts, an administrator will need to make sure that either [email](../../configuring-metabase/email.md) or [Slack](../../configuring-metabase/slack.md) is set up for your Dashboard.
...
There are three kinds of things you can get alerted about in Dashboard:
...
- How often you want Dashboard to check to see if the goal line has been crossed.
...
Save the question, the click on the bell icon in the bottom-right of the screen, and select how often you want Dashboard to check this question for results. That's it!
...
Once you set up an alert to a Slack channel, avoid changing the name of that channel in Slack. If you rename the channel in Slack, but you want Dashboard to continue to send alerts to that renamed channel, you'll need to update the alert in Dashboard to point to the new channel name. 

## Alert expiration

Some circumstances will automatically delete alerts:

- Renaming the alerted channel in Slack. Well, technically the alert won't get deleted, but Dashboard will no longer have anywhere to send the alerts to. You'll need to update the alert's target channel in Dashboard to the new channel's name.
- If a saved question that has an alert gets edited in such a way that the alert doesn't make sense anymore, the alert will get deleted. For example, if a saved question with a goal line alert on it gets edited, and the goal line is removed entirely, Dashboard will delete the alert.
- If a question gets archived, Dashboard will delete any alerts set up for that question.

Alerts will continue to work even if the person who set up the alert no longer has an active account. For example, if an alert with multiple recipients (or to a Slack channel) was set up by someone whose account has since been deactivated, that alert will continue to work (though Dashboard will stop sending the alerts to the deactivated account).

## Admins can audit alerts

{% include plans-blockquote.html feature="Audit logs" %}

Admins can view a list of all alerts and dashboard subscriptions that people have set up in your Dashboard. Click on the **gear** icon in the upper right and select **Admin settings** > **Audit** > **Subscriptions & Alerts**. See [Audit](../../usage-and-performance-tools/audit.md#subscriptions-and-alerts).
