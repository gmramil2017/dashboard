...
Dashboard lets you add and edit metadata to your tables and columns so that you can manage your org's business logic as it changes. Go to the **Table Metadata** tab in your **Admin settings** to:
...
> The **Table Metadata** admin settings only affect the way data's displayed and interpreted in Dashboard. None of the settings will change the data in your database.

## Table settings

Click on a table name in the left sidebar to view the table's settings in Dashboard.

- [Change the display name](#table-display-name).
- [Add or edit the description](#table-description).
- [Show or hide the table across Dashboard](#table-visibility).
- [View the original schema](#original-schema).
- [Edit column (field) settings](#column-field-settings).

If you've got more than one database connected to Dashboard, click on the database name (for example, "Sample Database") and select another database from the dropdown menu. Once you select a database, the tables in that database will appear in the sidebar.

### Table display name

To edit a table's display name in Dashboard, click into the box that contains the current table name. Changes will be saved automatically once you click out of the box.

### Table description

To add a table description, click into the box below the table name. Descriptions are displayed in Dashboard's [data reference](../exploration-and-organization/data-model-reference.md) to help people find the right table for their use case.

### Table visibility

**Queryable** tables are visible across all of Dashboard.
...
- [Show or hide the column across Dashboard](#column-visibility)
...
To change the _global_ display name of a column in Dashboard, click on the name of the column. For example, you could display "auth.user" as "User" to make the column more readable. People can use [models](./models.md) to give columns a display name that's local to the model.
...
Dashboard defaults to the column order defined in your database schema. To reorder the column display order in question results and menus **manually**, click on the grab bar to the right of each column, and drag the column to a new position.
...
- **Custom.** You choose the order. Dashboard will automatically switch the sort order to "Custom" if you rearrange any of the columns.
- **Smart.** Dashboard chooses for you.

### Field type

To change the [field type](../data-modeling/field-types.md) of a column, click on the **Type** dropdown menu in a column's setting box. You can also use the **Type** dropdown to label a column as an [entity key](https://www.metabase.com/glossary/entity_key) (primary key) or [foreign key](https://www.metabase.com/glossary/foreign_key) in Dashboard (with no consequence to your database).

### Casting to a specific data type

If you want Dashboard to treat a text or number column as a datetime column:
...
- **Search box**: Enter a search term and Dashboard will display checkboxes for column values that match the search.
- **A list of all values**: dropdown menu with checkboxes for all column values.
- **Plain input box**: Enter a search term and Dashboard will make autocomplete suggestions for the search (no checkboxes).
...
The dropdown filter widget can be finicky, because Dashboard needs to run a [scan](../databases/sync-scan.md#how-database-scans-work) to get the list of values for the dropdown menu.
...
When you change a default filter to a dropdown filter, you'll trigger a database query that gets the first 1,000 distinct values (ordered ascending) for that column. Dashboard will cache the first 100kB of text to display in the dropdown menu. If you have columns with more than 1,000 distinct values, or columns with text-heavy data, we recommend setting **Filtering on this field** to "Search box" instead.
