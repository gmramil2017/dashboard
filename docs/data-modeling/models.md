...
2. Choose either the query builder or a native query (if you want to use SQL). The advantage of using the query builder is that Dashboard will be able to fill out some of the metadata for you; if you use SQL, you'll have to fill out that metadata manually.
3. Select your data.
...
Metadata is the secret sauce of models. When you write a SQL query, Dashboard can display the results, but it can't "know" what kind of data it's returning (like it can with questions built using the query builder). What this means in practice is that people won't be able to drill-through the results, because Dashboard doesn't understand what the results are. With models, however, you can tell Dashboard what kind of data is in each returned column so that Dashboard can still do its drill-through magic. Metadata will also make filtering nicer by showing the correct filter widget, and it will help Dashboard to pick the right visulization for the results.

If you only set one kind of metadata, set the **Column type** to let Dashboard know what kind of data it's working with.
...
For models based on SQL queries, you can tell Dashboard if the column has the same type as an existing database column.
...
For [questions](../questions/start.md), [dashboards](../dashboards/start.md), and models, Dashboard keeps a version history for the previous fifteen versions of that item. You can view changes, revert to previous versions, and archive outdated items.
...
Dashboard can cache the results of your models so that the models load faster. Dashboard caches models by creating tables in a bespoke schema in your data warehouse, and saves the results of the queries that underlie your models in those tables. When people ask questions based on your models, Dashboard will substitute those cached results in place of running the model's query.

To set up model caching:

1. [Enable model caching in Dashboard](#enable-model-caching-in-metabase).
2. [Create a schema to store cached models](#create-a-schema-to-store-cached-models).

### Enable model caching in Dashboard
...
If the credentials you've given Dashboard to connect to your database are permissive, Dashboard should do all the work for you: Dashboard will check if the schema exists, or otherwise attempt to create it.
...
3. Ensure that the credentials Dashboard uses can manage and write to that schema.

### Refreshing a model's cached results

To refresh a model's cached results, go to the model and click on the **i** info icon. In the info sidebar that opens, you'll see a note about when Dashboard last refreshed the model's cache, and an icon to refresh the cache.
...
- [Models in Dashboard][learn-models]
