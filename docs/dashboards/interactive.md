...
By default, when you create charts using Dashboard's graphical query builder, your charts automatically come with [drill-through capabilities](https://www.metabase.com/learn/questions/drill-through), which let folks click on a chart to explore further. But if you have a more customized click path in mind, Dashboard allows you to customize what happens when a user clicks on a chart or table in your dashboard.
...
If you hover over the card that contains question you want to customize, Dashboard will display a menu at the top right of that card containing these options, from left to right:
...
Dashboard will slide out the **Click behavior sidebar**:
...
- Open the Dashboard drill-through menu.
...
Internal Dashboard destinations (dashboards or saved questions) will load in the same browser tab or window. External URLs will open in a new tab or window.
...
Once you select the column that contains the value you want to pass, the sidebar will display the column used to pass the value, as well as the target filter at the destination that Dashboard will pass the value to:

![Pass category to filter](./images/pass-category-to-filter.png)

In the example above, when a user clicks on the **Orders by product category** card, Dashboard will pass the clicked `Product -> Category` to the destination dashboard ("Interactive Dashboard"), which will then filter its cards by that `Category`.
