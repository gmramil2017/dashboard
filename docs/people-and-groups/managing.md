...
Click **Create** to activate an account. An account becomes active once you click **Create**, even if the person never signs into the account. The account remains active until you [deactivate the account](#deactivating-an-account). If you're on a paid Dashboard plan, all active accounts will count toward your user account total. If one person has more than one account, each account will count toward the total (see [how billing works](https://www.metabase.com/pricing/how-billing-works)).

If you’ve already [configured Dashboard to use email](../configuring-metabase/email.md), Dashboard will send the person an email inviting them to log into Dashboard. If you haven't yet setup email for your Dashboard, Dashboard will give you a temporary password that you’ll have to manually send to the person.
...
If you're using SSO, you should deactivate the account in Dashboard as well as your IdP (that is, deactivation doesn't get applied from Dashboard to your IdP, and vice versa).

![Remove a user](images/RemoveUser.png)

To reactivate a deactivated account, click the **Deactivated** radio button at the top of the people list to see the list of deactivated accounts. Click on the icon on the far right to reactivate that account, allowing them to log in to Dashboard again.

## Deleting an account

Dashboard doesn't explicitly support account deletion. Instead, Dashboard deactivates accounts so people can't log in to them, while it preserves any questions, models, dashboards, and other items created by those accounts.
...
- If they log in using Google credentials, Dashboard displays a Google icon.
- If they log in using an email address and password stored in Dashboard, no icon is shown.

Note that the type of user is set when the account is first created: if you create a user in Dashboard, but that person then logs in via Google or some other form of SSO, the latter's icon will _not_ show up next to their name.
...
If you're using Dashboard Cloud, [contact support](https://www.metabase.com/help-premium) to reset your admin password.

If you're a Dashboard admin and have access to the server console, you can get Dashboard to send you a password reset token:

1.  Stop the running Dashboard application.
2.  Restart Dashboard with `reset-password email@example.com`, where "email@example.com" is the email associated with the admin account:
    ```
    java -jar metabase.jar reset-password email@example.com
    ```
3.  Dashboard will print out a random token like this:
...
4.  Start Dashboard normally again (_without_ the `reset-password` option).
...
This action doesn't affect email distribution lists that are managed outside of Dashboard.
...
Every Dashboard has two default groups: Administrators and All Users. These are special groups that can’t be removed.

#### Administrators

To make someone an admin of Dashboard, you just need to add them to the Administrators group. Dashboard admins can log into the Admin Panel and make changes there, and they always have unrestricted access to all data that you have in your Dashboard instance. So be careful who you add to the Administrator group!

#### All users

The **All Users** group is another special one. Every Dashboard user is always a member of this group, though they can also be a member of as many other groups as you want. We recommend using the All Users group as a way to set default access levels for new Dashboard users. If you have [Google single sign-on](./google-and-ldap.md) enabled, new users who join that way will be automatically added to the All Users group.
...
- Add or remove people from their group (that is, people who already have accounts in your Dashboard).
...
Group managers are not admins, so their powers are limited. They cannot create new groups or invite new people to your Dashboard.
