...
2. [Configure SAML in Dashboard](./authenticating-with-saml.md#enabling-saml-authentication-in-metabase) (the service provider).
...
Now go to Settings tab inside Auth0's Dashboard App and fill in the following values:

- Application Login URI: the URL of your Dashboard instance.
- Allowed Callback URLs: the URL that's below **Configure your identity provider (IdP)** in SAML settings inside Dashboard (it ends with `/auth/sso/`).
...
### Mapping fields from users in Auth0 to Dashboard

On the top of the **Settings** section on this page, you need to re-enter the value you included in **Allowed Callback URLs** in the field named **Application Callback URL**. Copy and paste the following JSON file in the settings box so that Auth0 can send the correct mappings to Dashboard when a user logs in:
...
Next, in the Auth0 Addon: SAML2 Web App popup, click on the **Usage** tab and then configure in Dashboard (column on the left) the values that this tab provides (column on the right).
...
The "SAML Application Name" value can be left as the default (Dashboard).

Save your settings, then [enable SAML](authenticating-with-saml.md) in Dashboard, and you should be good to go!
