...
2. [Set up SAML in Dashboard](./authenticating-with-saml.md#enabling-saml-authentication-in-metabase) (the service provider).
...
5. Configure the SSO client from the form that appears after saving:
    ...
    - **Valid Redirect URIs**: URL where you are hosting your Dashboard instance followed by
     a slash (/) and an asterisk (*). For example, `http://localhost:3000/*`.
    - **Base URL**: Fill this in with the value under “URL the IdP should redirect back to” from your Dashboard **Admin settings** > **Authentication** > **SAML**.
    - Click **Save**.
6. Map user data to your SSO client from **Mappers** > **Add Builtin**.
    - [Mapping attributes from users in Keycloak to Dashboard](#mapping-attributes-from-users-in-keycloak-to-metabase)
7. Configure the service provider (Dashboard) from **Configure** > **Realm Settings**.
...
## Mapping fields from Keycloak to Dashboard

1. Go to your Dashboard **Admin settings** > **Authentication** > **SAML**.
...
## Mapping attributes from users in Keycloak to Dashboard

Keycloak can import four user attributes by default: name, surname, email and role.

Let's say we want email, name, and surname to be passed between the client (Dashboard) and the authentication server (Keycloak).
...
3. Click **Edit** beside each attribute and make the following changes:
    - **SAML Attribute Name**: the name that Dashboard expects to receive.
    - **SAML Attribute NameFormat**: select “Basic” from the dropdown menu.

You can find the attribute values from your Dashboard **Admin settings** > **Authentication** > **SAML** > **Attributes**.
