...
If you can't connect to your database, you'll need to figure out if the problem is happening with Dashboard or your database server.
...
## Troubleshooting connections to Dashboard

1. Go to **Admin** > **Databases** and select your database to confirm that your connection hasn’t been changed or deleted.

    - If Dashboard hasn't started syncing with your database, click **Sync database schema now**.

    - If Dashboard is taking a long time to sync, go to [Troubleshooting syncs and scans](./sync-fingerprint-scan.md).

2. Go to **Admin** > **Troubleshooting** > **Logs** to check if Dashboard failed to sync [due to an error](#common-database-connection-errors).

    - If the logs feel overwhelming, check out [How to read the server logs](./server-logs.md).

If you don't have access to the Dashboard Admin panel, you'll need to ask the person who set up your Dashboard.
...
2. Check if you can connect to the data warehouse from another client using the machine that you’re running Dashboard on.

    - If you can access the server from a bastion host or another machine, [check if your Dashboard's IP address has access to your database server](#checking-your-server-access).

    - If you're running Dashboard Cloud, check that you've [whitelisted our IP addresses](https://www.metabase.com/cloud/docs/ip-addresses-to-whitelist).

3. Make sure that Dashboard is using a role with the necessary privileges to connect to your data warehouse. See [Granting database privileges](../databases/users-roles-privileges.md).

The steps above will help you detect whether the problem is occurring outside of Dashboard. To _fix_ problems with your database server, you'll need to refer to the docs for your database or cloud service. Remember to [test your database connection](#testing-the-connection-status) after you make changes.
...
If you see this error message in the Dashboard interface, go to [Troubleshooting timeouts](./timeout.md).
...
To verify that your Dashboard's IP address can access the database server:
...
2. If you're running Dashboard Cloud, check that you've [whitelisted our IP addresses](https://www.metabase.com/cloud/docs/ip-addresses-to-whitelist).
...
- Hire a [Dashboard Expert](https://www.metabase.com/partners/){:target="_blank"}.
