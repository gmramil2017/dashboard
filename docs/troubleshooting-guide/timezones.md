...
4. What time zone is Dashboard using?
...
2. If you're using a database that doesn't support the report timezone setting, ensure that Dashboard's time zone matches that of the database. Dashboard's time zone is the Java Virtual Machine's time zone, typically set via a `-Duser.timezone<..>` parameter or the `JAVA_TIMEZONE` environment variable; exactly how it is set will depend on how you launch Dashboard. Note that Dashboard's time zone doesn't impact any databases that use a Report Time Zone.
...
2. If so, make sure the server time zone reflects the reporting time zone, because when a query is run on Dashboard, the server applies the configured time zone to that date.
