...
- [Running the Dashboard JAR][running].
- [Running Dashboard on Docker][docker].
- [Using or migrating from an H2 application database][appdb].

## Authentication

- [People can't log in to Dashboard][login].
- [LDAP][ldap].
- [SAML][saml].
...
- [The data in Dashboard doesn't match my database][sync-fingerprint-scan].
...
- [Dashboard isn't sending email][not-sending-email].
...
## Dashboard server and console logs

Dashboard will log errors, both on the server and in the browser console, depending on where the error occurs, which can help you track down an issue. Administrators will have access to the server logs, and everyone with a browser can open the developer tools to see the console logs.

**Accessing the Dashboard server logs**: You can look for the logs that Dashboard leaves on the server's file system (or wherever else you collect logs). If you're logged into Dashboard with an Admin account, you can also access the logs by going to the top right of the screen and clicking on the **gear** icon > **Admin settings** > **Troubleshooting** > **Logs**. Check out [How to read the server logs][server-logs].

**Checking for Javascript console errors:** Dashboard will send debugging information and errors to your browser's developer console. To open the developer console, follow the instructions for your web browser:
...
For tutorials that walk you through how to use Dashboard features, check out [Learn Dashboard][learn].
...
Dashboard adds new features and squashes bugs with each release. [Upgrading to the latest and greatest][upgrade] may resolve your issue. If you're using [Dashboard Cloud][cloud], we'll handle the upgrades for you. You can checkout the [release notes][releases] to see what's new.
