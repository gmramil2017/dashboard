     ...
     (try
       ...
       (let [path (u.files/get-path filename)]
         ...
         (assert (Files/isWritable path)
           (trs "Dashboard does not have permissions to write to plugins directory {0}" filename))
       ...
       (catch Throwable e
         (log/warn
          e
          (trs "Dashboard cannot use the plugins directory {0}" filename)
          "\n"
          (trs "Please make sure the directory exists and that Dashboard has permission to write to it.")
          (trs "You can change the directory Dashboard uses for modules by setting the environment variable MB_PLUGINS_DIR.")
         ...
         (let [path (u.files/get-path (System/getProperty "java.io.tmpdir"))]
           (assert (Files/isWritable path)
             (trs "Dashboard cannot write to temporary directory. Please set MB_PLUGINS_DIR to a writable directory and restart Dashboard."))
           {:path path, :temp true}))))))

(defn plugins-dir-info
  "Map with a :path key containing the `Path` to the Dashboard plugins directory, and a :temp key indicating whether a
  temporary directory was used."
  ^Path []
  (plugins-dir* (plugins-dir-filename)))

(defn plugins-dir
  "Get a `Path` to the Dashboard plugins directory, creating it if needed. If it cannot be created for one reason or
  ...
  (for [^Path path (u.files/files-seq (plugins-dir))
        :when      (and (u.files/regular-file? path)
                        ...
                        (or (not (str/ends-with? (.getFileName path) "spark-deps.jar"))
                            ...
                            ;; the Dashboard JAR itself as well as plugins no longer are; Java will throw an Exception
                            ...
                            (log/warn
                             (trs "spark-deps.jar is no longer needed by Dashboard 0.32.0+. You can delete it from the plugins directory."))))]
    path))
...
(defn- init-plugins! [paths]
  ...
  ;; driver `ojdbc8.jar`) always get initialized (i.e., added to the classpath) first; that way, Dashboard drivers that
  ;; depend on them (such as Oracle) can be initialized the first time we see them.
  ;;
  ;; In Clojure world at least `false` < `true` so we can use `sort-by` to get non-Dashboard-plugin JARs in front
...
(defn load-plugins!
  "Load Dashboard plugins. The are JARs shipped as part of Dashboard itself, under the `resources/modules` directory (the
  source for these JARs is under the `modules` directory); and others manually added by users to the Dashboard plugins
  directory, which defaults to `./plugins`.

  When loading plugins, Dashboard performs the following steps:

  *  Dashboard creates the plugins directory if it does not already exist.
  *  Any plugins that are shipped as part of Dashboard itself are extracted from the Dashboard uberjar (or `resources`
     directory when running with the Clojure CLI) into the plugins directory.
  *  Each JAR in the plugins directory that *does not* include a Dashboard plugin manifest is added to the classpath.
  *  For JARs that include a Dashboard plugin manifest (a `metabase-plugin.yaml` file), a lazy-loading Dashboard driver
