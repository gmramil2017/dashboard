(ns metabase.sync.sync-metadata
  "Logic responsible for syncing the metadata for an entire database.
   ...
   4.  Sync Dashboard Metadata table (`metabase.sync.sync-metadata.metabase-metadata`)"
  (:require
...
(s/defn sync-db-metadata!
  "Sync the metadata for a Dashboard `database`. This makes sure child Table & Field objects are synchronized."
