(ns metabase.cmd
  "Functions for commands that can be ran from the command-line with the Clojure CLI or the Dashboard JAR. These are ran
  as follows:
...
(defn- call-enterprise
  ...
  (let [f (try
            ...
            (resolve symb)
            (catch Throwable e
              (throw (ex-info (trs "The ''{0}'' command is only available in Dashboard Enterprise Edition." (name symb))
                              {:command symb}
                              e))))]
    (apply f args)))
...
(defn ^:command profile
  "Start Dashboard the usual way and exit. Useful for profiling Dashboard launch time."
  []
...
(defn ^:command help
  "Show this help message listing valid Dashboard commands."
  ([command-name]
  ...
  ([]
   ...
   (println "\tOpen an SQL shell for the Dashboard H2 DB")))

(defn ^:command version
  "Print version information about Dashboard and the current system."
  []
  (println "Dashboard version:" config/mb-version-info)
  (println "\nOS:"
...
(defn ^:command environment-variables-documentation
  "Generates a markdown file containing documentation for environment variables relevant to configuring Dashboard.
...
(defn ^:command load
  {:doc "Note: this command is deprecated. Use `import` instead.
         Load serialized Dashboard instance as created by [[dump]] command from directory `path`."
...
(defn ^:command import
  {:doc "Load serialized Dashboard instance as created by the [[export]] command from directory `path`."
...
(defn ^:command dump
  {:doc "Note: this command is deprecated. Use `export` instead.
         Serializes Dashboard instance into directory `path`."
...
(defn ^:command export
  {:doc "Serialize Dashboard instance into directory at `path`."
