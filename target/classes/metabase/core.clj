...
(comment
  ...
  ;; Make sure the custom Dashboard logger code gets loaded up so we use our custom logger for performance reasons.
  mb.logger/keep-me)
...
(log/info
 ...
 (format "\n\nCopyright © %d Dashboard, Inc." (.getYear (java.time.LocalDate/now)))

 (str "\n\n"
      (if config/ee-available?
        (str (deferred-trs "Dashboard Enterprise Edition extensions are PRESENT.")
             "\n\n"
             (deferred-trs "Usage of Dashboard Enterprise Edition features are subject to the Dashboard Commercial License.")
             " "
             (deferred-trs "See {0} for details." "https://www.metabase.com/license/commercial/"))
        (deferred-trs "Dashboard Enterprise Edition extensions are NOT PRESENT."))))
...
(defn- print-setup-url
  ...
  (let [hostname  (or (config/config-str :mb-jetty-host) "localhost")
    ...
    (log/info (u/format-color 'green
                              (str (deferred-trs "Please use the following URL to setup your Dashboard installation:")
                                   "\n\n"
                                   setup-url
                                   "\n\n")))))
...
(defn- destroy!
  ...
  []
  (log/info (trs "Dashboard Shutting Down ..."))
  ...
  (prometheus/shutdown!)
  (log/info (trs "Dashboard Shutdown COMPLETE")))
...
(defn- init!*
  ...
  []
  (log/info (trs "Starting Dashboard version {0} ..." config/mb-version-string))
  ...
  (log/info (trs "Setting up and migrating Dashboard DB. Please sit tight, this may take a minute..."))
  (mdb/setup-db!)
  ...
  ;; initialize Dashboard from an `config.yml` file if present (Enterprise Edition™ only)
  ...
  (let [start-time (.getStartTime (ManagementFactory/getRuntimeMXBean))
    ...
    (log/info (trs "Dashboard Initialization COMPLETE in {0}" (u/format-milliseconds duration)))))
...
(defn- start-normally []
  (log/info (trs "Starting Dashboard in STANDALONE mode"))
  (try
    ...
    (catch Throwable e
      (log/error e (trs "Dashboard Initialization FAILED"))
      (System/exit 1))))
...
(defn -main
  "Launch Dashboard in standalone mode."
  [& [cmd & args]]
  ...
  (if cmd
    ...
    (start-normally))) ; with no command line args just start Dashboard normally
