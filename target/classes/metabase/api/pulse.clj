...
(defn- maybe-filter-pulses-recipients
  "If the current user is sandboxed, remove all Dashboard users from the `pulses` recipient lists that are not the user
...
(defn- maybe-add-recipients
  ...
  merge in recipients that are Dashboard users, not raw email addresses, which these users can still view and modify."
