(ns ^:deprecated metabase.db.data-migrations
  ...
  Data migrations are run once when Dashboard is first launched.
  Note that there is no locking mechanism for data-migration - thus upon launching Dashboard, It's possible
  for a migration to be run multiple times (e.g: when running multiple Dashboard instances).
