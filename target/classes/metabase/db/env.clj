(ns metabase.db.env
  ...
  There are three ways you can specify application JDBC connection information for Dashboard:
  ...
  3. As a JDBC connection string (`MB_DB_CONNECTION_URI`) with username (`MB_DB_USER`) and/or password (`MB_DB_PASS`)
     passed separately. Support for this was added in Dashboard 0.43.0 -- see #20122.
...
(def ^:private h2-connection-properties
  ...
  {;; DB_CLOSE_DELAY=-1 = don't close the Database until the JVM shuts down
   ...
   :MVCC           true
   ;; Tell H2 to defrag when Dashboard is shut down -- can reduce DB size by multiple GIGABYTES -- see #6510
   :DEFRAG_ALWAYS  true
   ;; LOCK_TIMEOUT=60000 = wait up to one minute to acquire table lock instead of default of 1 second
   :LOCK_TIMEOUT   60000})

(defn- broken-out-details
  "Connection details that can be used when pretending the Dashboard DB is itself a `Database` (e.g., to use the Generic
  SQL driver functions on the Dashboard DB itself)."
...
(def env
  "Dashboard Datatbase environment. Used to setup *application-db* and audit-db for enterprise users."
...
(when (= db-type :h2)
  (log/warn
   (u/format-color
    ...
    (str/join
     " "
     ["WARNING: Using Dashboard with an H2 application database is not recommended for production deployments."
...
(when-let [raw-connection-string (not-empty (:mb-db-connection-uri env))]
  (when (and (= db-type :postgres)
    ...
    (log/warn (str/join " " ["Warning: Postgres connection string with `ssl=true` detected."
                             ...
                             "If Dashboard fails to launch, please add it and try again."
