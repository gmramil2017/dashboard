...
(defn- format-sql*
  ...
  Note that it will not play well with Dashboard parameters."
  [^String sql db-type]
  (when sql
...
(defmethod compile clojure.lang.IPersistentMap
  ...
  (let [sql-args (try
                   ...
                   (catch Throwable e
                     ...
                     ;; careful enough with the Honey SQL forms we create which is a bug in the Dashboard code we should
