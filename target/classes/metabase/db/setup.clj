...
(defn- print-migrations-and-quit-if-needed!
  ...
  (when (liquibase/has-unrun-migrations? liquibase)
    (log/info (str (trs "Database Upgrade Required")
                   "\n\n"
                   (trs "NOTICE: Your database requires updates to work with this version of Dashboard.")
                   "\n"
...
(s/defn ^:private verify-db-connection
  ...
  (let [error-msg (trs "Unable to connect to Dashboard {0} DB." (name db-type))]
...
(def ^:dynamic ^Boolean *disable-data-migrations*
  ...
  Dashboard is launched via `load-from-h2`.  That's because they will end up doing things like creating duplicate
