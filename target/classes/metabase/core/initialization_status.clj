...
(defn complete?
  "Is Dashboard initialized and ready to be served?"
  []
  (= @progress-atom 1.0))

(defn progress
  "Get the current progress of Dashboard initialization."
  []
  @progress-atom)

(defn set-progress!
  "Update the Dashboard initialization progress to a new value, a floating-point value between `0` and `1`."
  [^Float new-progress]
  {:pre [(float? new-progress) (<= 0.0 new-progress 1.0)]}
  (reset! progress-atom new-progress))

(defn set-complete!
  "Complete the Dashboard initialization by setting its progress to 100%."
  []
  (set-progress! 1.0))
