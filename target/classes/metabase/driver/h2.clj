...
(defmethod driver/connection-properties :h2
  ...
  (->>
   [{:name         "db"
     ...
     :helper-text (deferred-tru "The local path relative to where Dashboard is running from. Your string should not include the .mv.db extension.")
...
(defn- connection-string-set-safe-options
  "Add Dashboard Security Settings™ to this `connection-string` (i.e. try to keep shady users from writing nasty SQL)."
  [connection-string]
  {:pre [(string? connection-string)]}
