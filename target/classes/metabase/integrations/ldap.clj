...
(defsetting ldap-group-mappings
  ...
  ;; MB groups IDs
  (deferred-tru "JSON containing LDAP to Dashboard group mappings.")
  :type    :json
  :cache?  false
  :default {}
  :getter  (fn []
...
(def mb-settings->ldap-details
  "Mappings from Dashboard setting names to keys to use for LDAP connections"
  {:ldap-host                :host
   :ldap-port                :port
...
(s/defn fetch-or-create-user! :- (mi/InstanceOf User)
  "Using the `user-info` (from [[find-user]]) get the corresponding Dashboard user, creating it if necessary."
