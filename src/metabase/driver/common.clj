...
(def let-user-control-scheduling
  ...
  {:name         "let-user-control-scheduling"
   ...
   :description  (deferred-tru "By default, Dashboard does a lightweight hourly sync and an intensive daily scan of field values. If you have a large database, turn this on to make changes.")
   :visible-if   {"advanced-options" true}})
...
(def cache-field-values-schedule
  ...
  {:name "schedules.cache_field_values"
   ...
   :description  (deferred-tru
                   (str "Dashboard can scan the values present in each field in this database to enable checkbox "
                        "filters in dashboards and questions. This can be a somewhat resource-intensive process, "
                        "particularly if you have a very large database. When should Dashboard automatically scan "
...
(def refingerprint
  ...
  {:name         "refingerprint"
   ...
   :description  (deferred-tru
                   (str "This enables Dashboard to scan for additional field values during syncs allowing smarter "
...
(def default-options
  ...
  See the [plugin manifest reference](https://github.com/metabase/metabase/wiki/Dashboard-Plugin-Manifest-Reference)
  for more details."
  ...
  {:name   "cloud-ip-address-info"
   ...
   :getter (fn []
             (when-let [ips (public-settings/cloud-gateway-ips)]
               (str (deferred-tru
                      (str "If your database is behind a firewall, you may need to allow connections from our Dashboard "
