(ns metabase.sync.sync-metadata.tables
  "Logic for updating Dashboard Table models from metadata fetched from a physical DB."
  (:require
...
(s/defn ^:private our-metadata :- #{i/DatabaseMetadataTable}
  "Return information about what Tables we have for this DB in the Dashboard application DB."
...
(s/defn sync-tables-and-database!
  "Sync the Tables recorded in the Dashboard application database with the ones obtained by calling `database`'s driver's
