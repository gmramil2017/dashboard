  ...
  (let [currency-code (::mb.viz/currency format-settings "USD")]
    (condp = (::mb.viz/currency-style format-settings "symbol")
      ...
      (if (currency/supports-symbol? currency-code)
        ...
        ;; Fall back to using code if symbol isn't not supported on the Dashboard frontend
        currency-code)
...
(defmulti ^:private set-cell!
  ...
  This is based on the equivalent multimethod in Docjure, but adapted to support Dashboard viz settings."
...
(defn- add-row!
  ...
  This is based on the equivalent function in Docjure, but adapted to support Dashboard viz settings."
