...
(defn- current-major-version
  "Returns the major version of the running Dashboard JAR"
  []
  (second (extract-numbers (:tag config/mb-version-info))))

(defn rollback-major-version
  "Roll back migrations later than given Dashboard major version"
  ;; default rollback to previous version
  ([db-type conn liquibase]
   ;; get current major version of Dashboard we are running
