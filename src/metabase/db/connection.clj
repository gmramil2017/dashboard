...
(defn application-db
  "Create a new Dashboard application database (type and [[javax.sql.DataSource]]). For use in combination
  with [[*application-db*]]:
...
(def ^:dynamic ^ApplicationDB *application-db*
  "Type info and [[javax.sql.DataSource]] for the current Dashboard application database. Create a new instance
  with [[application-db]]."
...
(defn unique-identifier
  "Unique identifier for the Dashboard application DB. This value will stay the same as long as the application DB stays
