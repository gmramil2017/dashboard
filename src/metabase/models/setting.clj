...
(def ^:private retired-setting-names
  "A set of setting names which existed in previous versions of Dashboard, but are no longer used. New settings may not use
...
(def ^:private SettingDefinition
  {:name        s/Keyword
   ...
   :deprecated  (s/maybe s/Str)  ; if non-nil, contains the Dashboard version in which this setting was deprecated
    ...
    (if obfuscated?
      ...
      (do
        (when (and deprecated (not (nil? new-value)))
          (log/warn (trs "Setting {0} is deprecated as of Dashboard {1} and may be removed in a future version."
                         setting-name
                         deprecated)))
        (when (and
...
(defmacro defsetting
  ...
  If this setting is deprecated, this should contain a string of the Dashboard version in which the setting was
