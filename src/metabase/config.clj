...
;; https://github.com/metabase/metabase/wiki/Dashboard-Clojure-Style-Guide#dont-repeat-namespace-alias-in-function-names
;; for discussion
...
(def mb-version-info
  "Information about the current version of Dashboard. Comes from `version.properties` which is generated by the build
  script.
...
(def ^String mb-app-id-string
  "A formatted version string including the word 'Dashboard' appropriate for passing along
   with database connections so admins can identify them as Dashboard ones.
   Looks something like `Dashboard v0.25.0.RC1`."
  (str "Dashboard " (mb-version-info :tag)))

(defonce ^{:doc "This UUID is randomly-generated upon launch and used to identify this specific Dashboard instance during
...
(defonce
  ^{:doc "A string that contains identifying information about the Dashboard version and the local process."}
