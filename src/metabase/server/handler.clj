(ns metabase.server.handler
  "Top-level Dashboard Ring handler."
  (:require
...
(def ^:private middleware
  ...
  [#'mw.exceptions/catch-uncaught-exceptions    ; catch any Exceptions that weren't passed to `raise`
   ...
   #'mw.session/wrap-session-id                 ; looks for a Dashboard Session ID and assoc as :metabase-session-id
   #'mw.auth/wrap-api-key                       ; looks for a Dashboard API Key on the request and assocs as :metabase-api-key
