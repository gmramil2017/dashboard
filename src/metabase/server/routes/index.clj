...
(defn- load-template [path variables]
  (try
    ...
    (catch IllegalArgumentException e
      (let [message (trs "Failed to load template ''{0}''. Did you remember to build the Dashboard frontend?" path)]
        (log/error e message)
...
(defn- entrypoint
  "Response that serves up an entrypoint into the Dashboard application, e.g. `index.html`."
