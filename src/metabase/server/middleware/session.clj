...
;; How do authenticated API requests work? Dashboard first looks for a cookie called `metabase.SESSION`. This is the
...
;;
;; Finally we'll check for the presence of a `X-Dashboard-Session` header. If that isn't present, you don't have a
...
(defn clear-session-cookie
  "Add a header to `response` to clear the current Dashboard session cookie."
  [response]
...
(defn- merge-current-user-info
  ...
  (merge
   ...
   (when x-metabase-locale
     (log/tracef "Found X-Dashboard-Locale header: using %s as user locale" (pr-str x-metabase-locale))
