...
(defsetting slack-token
  (deferred-tru
    (str "Deprecated Slack API token for connecting the Dashboard Slack bot. "
...
(defsetting slack-app-token
  (deferred-tru
    (str "Bot user OAuth token for connecting the Dashboard Slack app. "
         "This should be used for all new Slack integrations starting in Dashboard v0.42.0.")))
...
(defsetting slack-files-channel
  (deferred-tru "The name of the channel to which Dashboard files should be initially uploaded")
...
(s/defn join-channel!
  "Given a channel ID, calls Slack API `conversations.join` endpoint to join the channel as the Dashboard Slack app.
