...
(let [f        (fn []
                 ...
                 (assert ((requiring-resolve 'metabase.db/db-is-set-up?)) "Dashboard DB is not yet set up")
...
(define-premium-feature hide-embed-branding?
  "Logo Removal and Full App Embedding. Should we hide the 'Powered by Dashboard' attribution on the embedding pages?
...
(defsetting is-hosted?
  "Is the Dashboard instance running in the cloud?"
  :type       :boolean
  :visibility :public
  :setter     :none
...
(defmacro defenterprise
  "Defines a function that has separate implementations between the Dashboard Community Edition (aka OSS) and
