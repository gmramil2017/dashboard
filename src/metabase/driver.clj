(ns metabase.driver
  "Dashboard Drivers handle various things we need to do with connected data warehouse databases, including things like
...
(defn available?
  ...
  when Dashboard starts up (meaning this will return `true` for them) and only initialized when first needed."
  [driver]
...
(defmulti initialize!
  ...
  about the driver, but do not connect to databases; these can be be supplied, for example, by a Dashboard plugin
  ...
  Dashboard drivers in v1.0 and above) are automatiaclly given an implentation of this method that performs the
...
(defmulti supports?
  ...
  DEPRECATED — [[database-supports?]] should be used instead. This function will be removed in Dashboard version 0.50.0."
...
(defmulti mbql->native
  ...
  For example, a driver like Postgres would build a valid SQL expression and return a map such as:
    ...
    {:query \"-- Dashboard card: 10 user: 5
...
(defmulti superseded-by
  ...
  deprecated in Dashboard and will eventually be replaced by the returned driver, in some future version (at which point
