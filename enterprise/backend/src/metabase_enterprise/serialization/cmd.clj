...
(s/defn v1-load
  ...
  (when-not (load/compatible? path)
    (log/warn (trs "Dump was produced using a different version of Dashboard. Things may break!")))
...
(mu/defn v2-load
  ...
  ;(when-not (load/compatible? path)
  ;  (log/warn (trs "Dump was produced using a different version of Dashboard. Things may break!")))
  (log/info (trs "Loading serialized Dashboard files from {0}" path))
  (serdes/with-cache
...
(defn v1-dump
  "Legacy Dashboard app data dump"
...
(defn v2-dump
  "Exports Dashboard app data to directory at path"
  [path {:keys [user-email collection-ids] :as opts}]
  (log/info (trs "Exporting Dashboard to {0}" path) (u/emoji "🏭 🚛💨"))
  (mdb/setup-db!)
  (t2/select User) ;; TODO -- why??? [editor's note: this comment originally from Cam]
  (serdes/with-cache
    (-> (cond-> opts
