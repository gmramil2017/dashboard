(ns ^{:added "0.45.0"} metabase-enterprise.advanced-config.file
  "Support for initializing Dashboard with configuration from a `config.yml` file located in the current working
  ...
  Config files are required to have a `version` key; each version of Dashboard that supports config files (i.e., 45 and
  ...
  without having older Dashboard code suddenly break in mysterious ways because it doesn't understand the new config
  shape, or newer Dashboard code breaking if you try to use a config file using the older shape.

  For the time being, the minimum version we'll support is 1.0, which is the initial version of the config spec that
  we're shipping with Dashboard 45. We'll support all the way up to `1.999` (basically anything less than `2.0`). This
  will give us some room to define new backwards-compatible versions going forward.

  For example in Dashboard 46 if we want to add some extra required keys that Dashboard 45 can safely ignore, we can
  define a new version 1.1 of the spec and specify Dashboard 46 works with config versions `1.1` to `1.999`.
...
(defn- path
  "Path for the YAML config file Dashboard should use for initialization and Settings values."
  ^java.nio.file.Path []
  (let [path* (or (some-> (get *env* :mb-config-file-path) u.files/get-path)
    ...
    (if (u.files/exists? path*)
      (log/info (u/colorize :magenta
                            (trs "Found config file at path {0}; Dashboard will be initialized with values from this file"
                                 (pr-str (str path*))))
                (u/emoji "🗄️"))
...
(defn ^{:added "0.45.0"} initialize!
  "Initialize Dashboard according to the directives in the config file, if it exists."
  []
    ...
    (doseq [[section-name section-config] (sort-by-initialization-order (:config m))]
      ...
      (when-not (= section-name :settings)
        (when-not (premium-features/has-feature? :advanced-config)
          (throw (ex-info (tru "Dashboard config files require a Premium token with the :advanced-config feature.")
                          {}))))
