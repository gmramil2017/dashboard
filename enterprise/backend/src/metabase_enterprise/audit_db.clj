...
(defn- install-database!
  ...
  - In the unlikely case that a user has many many databases in Dashboard, and ensure there can Never be a collision, we
   ...
   (if (t2/select-one Database :id id)
     ...
     (do
       ...
       (t2/insert! Database {:is_audit         true
                             ...
                             :name             "Internal Dashboard Database"
...
(def analytics-root-dir-resource
  "Where to look for analytics content created by Dashboard to load into the app instance on startup."
