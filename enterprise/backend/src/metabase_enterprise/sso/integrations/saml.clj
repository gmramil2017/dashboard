(ns metabase-enterprise.sso.integrations.saml
  ...
  3. Dashboard issues another redirect to the identity provider URI

  4. User logs into their identity provider (i.e. Auth0)

  5. Identity provider POSTs to Dashboard with successful auth info

  6. Dashboard parses/validates the SAML response

  7. Dashboard inits the user session, responds with a redirect to back to the original `url`"
  (:require
