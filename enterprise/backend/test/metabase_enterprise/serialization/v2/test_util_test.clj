...
(deftest mbql-deserialize-test
  (mt/with-empty-h2-app-db
    (ts/with-temp-dpc [Database   [{db-id      :id} {:name "Dashboard Store"}]
      ...
      (is (= {:database db-id
             ...
             (#'serdes/mbql-fully-qualified-names->ids
               {:database "Dashboard Store",
                :type     "query",
                :query    {:source-table ["Dashboard Store" "public" "crm_survey_response"],
                           :aggregation  [["cum-count"]],
                           :breakout     [["datetime-field" ["field-id" ["Dashboard Store" "public"
                                                                         "crm_survey_response" "created_at"]] "week"]],
                           :filter       ["<" ["field-id" ["Dashboard Store" "public" "crm_survey_response" "nps"]] 9]}}))))))
