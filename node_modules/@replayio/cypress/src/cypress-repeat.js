...
function CypressRepeat({ repeat = 1, mode = SpecRepeatMode.All, untilPasses = false, args = [], timeout, }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let [k, runOptions] of allRunOptions.entries()) {
            ...
            let testResults;
            try {
                testResults = yield new Promise((resolve, reject) => {
                    const child = (0, child_process_1.fork)(path_1.default.join(__dirname, "cypress-run.js"));
                    child.send(runOptions);
                    child.on("message", msg => {
                        const resp = msg.valueOf();
                        if (resp.success) {
                            resolve(resp.data);
                        }
                        else {
                    ...
                    if (timeout) {
                        setTimeout(() => {
                            if (!testResults) {
                                (0, terminate_1.default)(child.pid, "SIGINT", { timeout: 1000 }, () => {
                                    reject("Timed out");
                                });
                            }
                        }, timeout);
