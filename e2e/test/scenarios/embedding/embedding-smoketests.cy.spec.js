...
const licenseExplanations = [
  `When you embed charts or dashboards from Dashboard in your own application, that application isn't subject to the Affero General Public License that covers the rest of Dashboard, provided you keep the Dashboard logo and the "Powered by Dashboard" visible on those embeds.`,
...
describe("scenarios > embedding > smoke tests", () => {
  ...
  context("embedding disabled", () => {
    ...
    it("should display the embedding page correctly", { tags: "@OSS" }, () => {
      ...
      if (isOSS) {
        cy.contains(
          "In order to remove the Dashboard logo from embeds, you can always upgrade to one of our paid plans.",
        );
  ...
  context("embedding enabled", { tags: "@OSS" }, () => {
    ["question", "dashboard"].forEach(object => {
      it(`should be able to publish/embed and then unpublish a ${object} without filters`, () => {
        ...
        if (isEE) {
          cy.contains("Powered by Dashboard").should("not.exist");
        } else {
          cy.contains("Powered by Dashboard")
            .closest("a")
